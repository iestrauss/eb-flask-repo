...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////usr/local/var/apehouse.db'
...
##############################################
@app.route('/votefor/<int:article_id>', methods=['GET', 'POST'])
...
#    article_vote_list = ArticleVote.query.filter_by(article_id=article_id)
    ...
    article_vote_list = db.session.query(ArticleVote, User).filter(ArticleVote.article_id == article_id).filter(ArticleVote.user_id == User.fb_id).all()
...
if __name__ == '__main__':
    db.create_all()
    app.run(host="0.0.0.0", debug=True, ssl_context=("example.com+2.pem", "example.com+2-key.pem"))
